...
const AppSettingItem: React.FC<AppSettingItemProps> = ({ label, subLable, icon, arrowForward, settingStatus }) => {
    return (
        <TouchableOpacity style={{ ...commonStyles.rowWithSpaceBetween, minHeight: 52 }}>
            ...
            <View style={{ ...commonStyles.row, columnGap: 5 }}>
                {settingStatus && <Text style={styles.notificationStatus}>{settingStatus}</Text>}
                {arrowForward && <Image source={icons.icon_arrow_forward_line_30} style={{ width: 16, height: 16 }} />}
            </View>
        </TouchableOpacity>
    );
};
